...
class Settings(metaclass=Singleton):
    ...
    databases = json.loads(CLICKHOUSE_MIGRATE_DATABASES) if CLICKHOUSE_MIGRATE_DATABASES else None
    migration_dir = CLICKHOUSE_MIGRATE_DIRECTORY

    def init_config(self, config_path: Optional[str] = CONFIG_PATH, databases: Optional[str] = None, migration_dir: Optional[str] = DEFAULT_MIGRATIONS_DIR):
        if self.databases is None:

            if not databases:
                databases, migration_dir = self.__get_params_from_config_file(config_path=config_path)
    ...
    def __get_params_from_config_file(config_path: str) -> Tuple[str, List[str]]:
        try:
            ...
            return json.loads(config.get("databases", "connection_strings")), json.loads(config.get("migration_path", "directory"))
        except KeyError:
    ...
    def __post_init_checks(self):
        if self.databases is None:
            raise ConfigError("Unable to configure migration tool. None of the configuration options were provided")
        if not path.isdir(self.migration_dir):
            raise ConfigError("Unable to configure migration tool. Provided migration dir does not exist")
